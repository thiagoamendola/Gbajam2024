...
template <const fr::model_3d_item &model_3d_item_ref>
class static_model_3d_item
{
public:
	...
	constexpr static_model_3d_item(fr::point_3d position, bn::fixed theta)
		: _vertices(_create_array<fr::vertex_3d, vertices_count>(
		  ...
		  _faces(
			  _create_array<fr::face_3d, faces_count>(model_3d_item_ref.faces()[0]))
	{
		...
		_vertical_cylinder = fr::model_3d_vertical_cylinder(position.x(), position.z(), cylinder_radius);

		for (int index = 0; index < vertices_count; ++index)
		{
			fr::vertex_3d transformed_vertex(model.transform(input_vertices[index]));
			_vertices[index] = transformed_vertex;
		}

		const bn::span<const fr::face_3d> &input_faces = model_3d_item_ref.faces();
	...
	{
		// static_assert(_vertices.size() < -1, "E no compiling time? vertices when
		// building thingy = "); //, input_vertices.size());
		return fr::model_3d_item(_vertices, _faces, model_3d_item_ref.collision_face(),
								 &_vertical_cylinder);
	}
...
private:
	...
	[[nodiscard]] static constexpr bn::array<Type, Size>
	_create_array(const Type &value)
	{
