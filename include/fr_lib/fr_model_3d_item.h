...
class face_3d
{
  ...
  public:
    ...
    constexpr face_3d(const bn::span<const vertex_3d> &vertices,
        ...
        : _centroid(_calculate_centroid(vertices, first_vertex_index,
                                        second_vertex_index,
                                        third_vertex_index)),
          _normal(normal), _first_vertex_index(first_vertex_index),
          _second_vertex_index(second_vertex_index),
          _third_vertex_index(third_vertex_index),
          _fourth_vertex_index(first_vertex_index), _color_index(color_index),
          _shading(_calculate_shading(shading, normal.point().y())),
          _triangle(true)
    {
        BN_ASSERT(color_index >= 0 && color_index < max_colors,
    ...
    constexpr face_3d(const bn::span<const vertex_3d> &vertices,
        ...
        : _centroid(_calculate_centroid(vertices, first_vertex_index,
          ...
          _second_vertex_index(second_vertex_index),
          _third_vertex_index(third_vertex_index),
          _fourth_vertex_index(fourth_vertex_index), _color_index(color_index),
          _shading(_calculate_shading(shading, normal.point().y())),
          _triangle(false)
    {
        BN_ASSERT(color_index >= 0 && color_index < max_colors,
  ...
  private:
    ...
    [[nodiscard]] constexpr static vertex_3d _calculate_centroid(
        const bn::span<const vertex_3d> &vertices, int first_vertex_index,
        int second_vertex_index, int third_vertex_index)
    {
        BN_ASSERT(vertices.size() > 0 && vertices.size() < 32768,
                  "Invalid vertices count: ", vertices.size());
        BN_ASSERT(first_vertex_index >= 0 &&
                      first_vertex_index < vertices.size(),
                  "Invalid first vertex index: ", first_vertex_index, " - ",
                  vertices.size());
        BN_ASSERT(second_vertex_index >= 0 &&
                      second_vertex_index < vertices.size(),
                  "Invalid second vertex index: ", second_vertex_index, " - ",
                  vertices.size());
        BN_ASSERT(third_vertex_index >= 0 &&
                      third_vertex_index < vertices.size(),
                  "Invalid third vertex index: ", third_vertex_index, " - ",
                  vertices.size());

        const point_3d &first_point =
            vertices.data()[first_vertex_index].point();
        const point_3d &second_point =
            vertices.data()[second_vertex_index].point();
        const point_3d &third_point =
            vertices.data()[third_vertex_index].point();
        ...
        int fourth_vertex_index)
    {
        ...
        BN_ASSERT(third_vertex_index >= 0 &&
                  ...
                  "Invalid third vertex index: ", third_vertex_index, " - ",
                  vertices.size());
        BN_ASSERT(fourth_vertex_index >= 0 &&
                      fourth_vertex_index < vertices.size(),
                  "Invalid fourth vertex index: ", fourth_vertex_index, " - ",
                  vertices.size());
        ...
        const point_3d &third_point =
            vertices.data()[third_vertex_index].point();
        const point_3d &fourth_point =
            vertices.data()[fourth_vertex_index].point();
        BN_ASSERT(first_point != second_point, "Vertices are the same");
        BN_ASSERT(first_point != third_point, "Vertices are the same");
        BN_ASSERT(second_point != third_point, "Vertices are the same");
        ...
        return vertex_3d(
            (first_point + second_point + third_point + fourth_point) / 4);
    }

    [[nodiscard]] constexpr static int _calculate_shading(int input_shading,
                                                          bn::fixed normal_y)
    {
        ...
        BN_ASSERT(input_shading >= 0 && input_shading <= 7,
                  "Invalid shading: ", input_shading);
...
class model_3d_vertical_cylinder
{
  ...
  public:
    ...
    constexpr model_3d_vertical_cylinder(bn::fixed centroid_x,
                                         bn::fixed centroid_z,
                                         int integer_radius)
        : _centroid_x(centroid_x), _centroid_z(centroid_z),
          _integer_radius(integer_radius)
    {
        BN_ASSERT(integer_radius >= 0,
                  "Invalid integer radius: ", integer_radius);
    }
...
class model_3d_item
{
  ...
  public:
    constexpr model_3d_item(const bn::span<const vertex_3d> &vertices,
        ...
        : model_3d_item(vertices, faces, nullptr, nullptr, nullptr)
    {
    }
    ...
    constexpr model_3d_item(const bn::span<const vertex_3d> &vertices,
        ...
        : model_3d_item(vertices, faces, nullptr, nullptr, palette)
    {
    }
    ...
    constexpr model_3d_item(const bn::span<const vertex_3d> &vertices,
        ...
        : model_3d_item(vertices, faces, collision_face, nullptr, nullptr)
    {
    }
    ...
    constexpr model_3d_item(const bn::span<const vertex_3d> &vertices,
                            const bn::span<const face_3d> &faces,
                            const face_3d *collision_face,
                            const model_3d_vertical_cylinder *vertical_cylinder)
        : model_3d_item(vertices, faces, collision_face, vertical_cylinder,
                        nullptr)
    {
    }
    ...
    constexpr model_3d_item(const bn::span<const vertex_3d> &vertices,
                            ...
                            const model_3d_vertical_cylinder *vertical_cylinder,
                            const bn::color *palette)
        : _vertices(vertices), _faces(faces), _collision_face(collision_face),
          _vertical_cylinder(vertical_cylinder), _palette(palette)
    {
        BN_ASSERT(vertices.size() > 0 && vertices.size() < 32768,
                  "Invalid vertices count: ", vertices.size());
    ...
    [[nodiscard]] constexpr const model_3d_vertical_cylinder *
    vertical_cylinder() const
    {
...
class face_texture
{
  ...
  public:
    constexpr face_texture(const bn::sprite_palette_item &palette_item,
                           const bn::sprite_tiles_item &small_tiles_item,
                           const bn::sprite_tiles_item &normal_tiles_item,
                           const bn::sprite_tiles_item &big_tiles_item,
                           const bn::sprite_tiles_item &huge_tiles_item)
        : _palette_item(palette_item), _small_tiles_item(small_tiles_item),
          _normal_tiles_item(normal_tiles_item),
          _big_tiles_item(big_tiles_item), _huge_tiles_item(huge_tiles_item)
    {
    }
    ...
    [[nodiscard]] constexpr const bn::sprite_tiles_item &small_tiles_item()
        const
    {
        return _small_tiles_item;
    }

    [[nodiscard]] constexpr const bn::sprite_tiles_item &normal_tiles_item()
        const
    {
