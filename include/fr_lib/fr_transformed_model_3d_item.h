...
namespace fr
{
    ...
    public:
        constexpr transformed_model_3d_item(bn::fixed x, bn::fixed z, bn::fixed theta) : _vertices(_create_array<vertex_3d, vertices_count>(model_3d_item_ref.vertices()[0])),
                                                                                         _faces(_create_array<face_3d, faces_count>(model_3d_item_ref.faces()[0]))
        {
            ...
            // static_assert(_vertices.size() < -1, "E no compiling time? _vertices = "); //, input_vertices.size());
        ...
        {
            // static_assert(_vertices.size() < -1, "E no compiling time? vertices when building thingy = "); //, input_vertices.size());
