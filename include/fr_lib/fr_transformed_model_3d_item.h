...
class transformed_model_3d_item
{
  ...
  public:
    constexpr transformed_model_3d_item(bn::fixed x, bn::fixed z,
                                        bn::fixed theta)
        : _vertices(_create_array<vertex_3d, vertices_count>(
              model_3d_item_ref.vertices()[0])),
          _faces(
              _create_array<face_3d, faces_count>(model_3d_item_ref.faces()[0]))
    {
        const bn::span<const vertex_3d> &input_vertices =
        ...
        for (int index = 0; index < faces_count; ++index)
        {
            ...
            if (input_face.triangle())
            {
                _faces[index] = face_3d(
                    ...
                    input_face.shading()); // <-- HERE for dynamic???
            }
            else
            {
                _faces[index] = face_3d(
    ...
    {
        // <-- Be wary I might need to add support to this as well
        return model_3d_item(_vertices, _faces,
